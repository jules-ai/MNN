...
int main(int argc, const char* argv[]) {
    if (argc < 3) {
        MNN_PRINT("Usage: %s model.mnn input0.png\n",argv[0]);
        return 0;
    }
    ...
    auto bits_        = inputUser->getType().bits;
    MNN_PRINT("input: w:%d , h:%d, bpp: %d bits: %u\n", size_w, size_h, bpp, bits_);
    for (int batch = 0; batch < shape[0]; ++batch){
        ...
        auto ts1 = getTimeInUs();
        MNN_PRINT("Image bits val: %u %u %u %f by time %ldus\n", *((uint8_t*)inputImage),*((uint8_t*)inputImage+1),*((uint8_t*)inputImage+2), *((float*)(inputUser->host<uint8_t>())), ts1-ts0);
        stbi_image_free(inputImage);
        //::memset(inputUser->host<uint8_t>(),0,128*128*3);
    }
        auto ts2 = getTimeInUs();
    input->copyFromHostTensor(inputUser.get());
        auto ts3 = getTimeInUs();
    if (false) {
        std::ofstream outputOs("output_0.txt");
        ...
        auto ts4 = getTimeInUs();
    net->runSession(session);
        auto ts5 = getTimeInUs();
    auto dimType = output->getDimensionType();
    if (output->getType().code != halide_type_float) {
        dimType = Tensor::TENSORFLOW;
    }
    std::shared_ptr<Tensor> outputUser(new Tensor(output, dimType));
        auto ts6 = getTimeInUs();
    output->copyToHostTensor(outputUser.get());
        auto ts7 = getTimeInUs();
        MNN_PRINT("time consume: copy %ld us, infer %ld us copy %ld us\n", ts3-ts2, ts5-ts4, ts7-ts6);
